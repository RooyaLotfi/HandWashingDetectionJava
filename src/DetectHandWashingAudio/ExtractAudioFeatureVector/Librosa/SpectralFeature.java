package DetectHandWashingAudio.ExtractAudioFeatureVector.Librosa;
 ...
 *
 * @author ChiaChun FU
 * @updates by Roya Lotfi
 *
 */


    private static int       n_mfcc       		= 20;
    private static double    fMin                 = 0.0;
    private static int       n_fft                = 2048;
    private static int       hop_length           = 2048;
    private static int	   n_mels               = 128;

    private static double    sampleRate           = 44100;
    private static double    fMax                 = sampleRate/2.0;
...
//	MFCC(int n_mfcc, double fMin, int hop_length, int n_mels, double sampleRate, double fMax){
//		this.n_mfcc = n_mfcc;
//		this.fMin = fMin;
...
//	}
    ...
    //summing over columns of mfcc
    public double[] mfcc_coeffs(double[] doubleInputBuffer) {
        final double[][] mfccResult = dctMfcc(doubleInputBuffer);
        double[] finalMFCC = new double[n_mfcc];
        int k = 0;
        int col_sum =0;
        for( int i=0; i < n_mfcc ; i++){
            col_sum = 0;
            for(int j = 0 ; j < mfccResult[0].length ; j ++){
                finalMFCC[k] = mfccResult[i][j] + col_sum;
            }
            k = k+1;
        }
        return finalMFCC;
    ...
    public double[] spectralCentroid(double[] y){
        //double[][] spectro = stftMagSpec(doubleInputBuffer);
//		System.out.println("size of spectro length    "+ spectro.length);
//		System.out.println("size of spectro 0    "+ spectro[0].length);
//		System.out.println("size of spectro 1    "+ spectro[1].length);
//		System.out.println("size of spectro 1    "+ spectro[spectro.length-1].length);
        ...
        double[] specCent = new double[frame[0].length];
        ...
        for (int k = 0; k < frame[0].length; k++){
            ...
            //System.out.println("size of mag spec _________"+Arrays.toString(magSpec));
            //System.out.println("frame number is k _________" + k);
            //System.out.println("frame number is k _________" + frame[0].length);

            tempSum = 0;
            magSum = 0;
            for (int i =0; i < magSpec.length; i++){
                tempSum = tempSum + magSpec[i]*fftFrame[i];
                magSum = magSum + magSpec[i];
            }
            specCent[k] = (double)(tempSum/magSum);
            //System.out.println("single spec is is "+specCent[k]);
        }

        //System.out.println("Spectral centroid is  "+Arrays.toString(specCent));
        //System.out.println("length of Spectral centroid is  "+specCent.length);
        return specCent;
    ...
    private double[][] stftMagSpec(double[] y){
        ...
        //System.out.println(fftmagSpec.length);
        return fftmagSpec;
    }

    private double[] magSpectrogram(double[] frame){
        double[] magSpec = new double[frame.length];
        //System.out.println("size of array is"+frame.length+"arrays is " + Arrays.toString(frame));
        fft.process(frame);
        for (int m = 0; m < frame.length; m++) {
            magSpec[m] = fft.real[m] * fft.real[m] + fft.imag[m] * fft.imag[m];
        }
        //System.out.println(magSpec.length+" ln of magnitude spectrum "+Arrays.toString(magSpec));
        return magSpec;
    }
    ...
    public double[] fftFreq() {
        ...
        //System.out.println(freqs.length+" frequency length "+Arrays.toString(freqs));
        return freqs;
    }
